...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 zorin-os --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="zorin-os"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
...
if [ ! -d "$ISO_PATH" ]; then
    msg_info "Standard ISO path not found. Attempting to create."
    mkdir -p $ISO_PATH
fi
